...
interface SendMoneyNotificationData {
    amount: string;
    currency: string;
    toAddress: string;
    txHash: string;
...
interface ReceiveMoneyNotificationData {
    amount: string;
    currency: string;
    fromAddress: string;
    txHash: string;
...
interface SwapNotificationData {
    fromAmount: string;
    fromCurrency: string;
    toAmount: string;
    toCurrency: string;
...
interface AuthContextType {
    ...
    createSendMoneyNotification: (data: SendMoneyNotificationData) => Promise<{ message: string }>;
    createReceiveMoneyNotification: (data: ReceiveMoneyNotificationData) => Promise<{ message: string }>;
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const createSendMoneyNotification = async (
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to create send money notification: ${response.status}`);
            }
    ...
    const createReceiveMoneyNotification = async (
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to create receive money notification: ${response.status}`);
            }
    ...
    const createSwapNotification = async (
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to create swap notification: ${response.status}`);
            }
