...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    const login = async (email: string, password: string): Promise<boolean> => {
        try {
            ...
            if (!authRes.ok) {
                const errorMessage =
                    authData.error || authData.message || 'Login failed';
            ...
            console.log(
                'Received token:',
                receivedToken ? 'Token found' : 'No token'
            );
    ...
    const getWalletAddresses = async (): Promise<WalletAddress[]> => {
        if (!token) {
            throw new Error(
                'Authentication required to fetch wallet addresses'
            );
        }
    ...
    const markAllNotificationsAsRead = async (): Promise<{
        message: string;
    }> => {
        if (!token) {
            throw new Error(
    ...
    const getTransactionHistory = async (
        ...
        if (!token) {
            throw new Error(
                'Authentication required to fetch transaction history'
            );
        }
